...
class User < ApplicationRecord
  ...
  def accepted_sent_follow_requests
    self.class.joins(:sent_follow_requests).where({ :follow_requests => { :status => 'accepted', :sender_id => self.id } })
  end
  ...
  def accepted_received_follow_requests
    self.class.joins(:received_follow_requests).where({ :follow_requests => { :status => 'accepted', :recipient_id => self.id } })
  end
  ...
  def feed
    Photo.where({ :user_id => self.leaders.select(:id) })
  end

  def discover
    Photo.joins(:likes).where({ :likes => { :user_id => self.leaders.select(:id) } })
  end
